...
class AssistantMessageTest < ActiveSupport::TestCase
  ...
  test "append_text! streams into a pending bubble" do
    ...
    # Claimed as `generating`, not `complete`: the turn is still streaming and
    # Assistant::Builtin settles it once the responder finishes, which is what
    # holds the composer's send-lock for the whole turn.
    assert message.generating?
  end
