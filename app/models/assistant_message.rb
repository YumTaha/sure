class AssistantMessage < Message
  ...
  private
    ...
    # Flips pending -> generating with a conditional UPDATE, so the check and the
    ...
    #
    # `generating` (not `complete`) is the claimed state: the turn is still
    # streaming, and Assistant::Builtin settles it to `complete`/`failed` once
    # the responder finishes. That keeps the send-lock held for the whole turn.
    #
    ...
    def claim!
      ...
      claimed = self.class.where(id: id, status: :pending).update_all(status: "generating").positive?
      self.status = :generating if claimed
      claimed
    end
end
